...
final class WeatherAPIService: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    func fetchWeather() async {
        ...
        guard let url = URL(string: "https://api.tomorrow.io/v4/timelines?location=\(location.coordinate.latitude),\(location.coordinate.longitude)&fields=temperature&fields=humidity&fields=uvIndex&fields=weatherCode&units=metric&timesteps=1h&startTime=\(dateFormatter.string(from: Date()))&endTime=\(dateFormatter.string(from: Date().addingTimeInterval(60 * 60)))&apikey=LmQ4iDdc5HzTvyxk0XY9JMmfIpYl2HJX") else {
            return
        }
               ...
               let weatherCode = WeatherCode(rawValue: "\(weatherValue.weatherCode)") {
                DispatchQueue.main.async { [weak self] in
                    self?.currentWeather = Weather(temperature: Int(weatherValue.temperature),
                                                   humidity: Int(weatherValue.humidity),
                                                   uvIndex: Int(weatherValue.uvIndex),
